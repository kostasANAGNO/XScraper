...
class TwitterExtractor:
    ...
    def search_tweets(self, keywords, start_date, end_date):
        for keyword in keywords:
            # Clear the data list for the new keyword
            self.data = []
            
            if isinstance(keyword, list):
                keyword_query = ' OR '.join([kw for kw in keyword])
            else:
            ...
            delta = timedelta(hours=1)  # Use of 30 minutes gap
            ...
            while current_date < end_date_dt:
                ...
                logger.info(f"Searching tweets for keyword {keyword_query} from {start_str} to {end_str}")

                try:
                    self.driver.get(f"https://twitter.com/search?q=({keyword_query})%20since%3A{start_str}%20until%3A{end_str}&src=typed_query&f=live")
                ...
                time.sleep(300)  # Add delay between each request within the same extraction

            logger.info(f"Completed searching for keyword {keyword_query}, saving data and sleeping before next hashtag.")
            self._save_to_json('_'.join(keyword) if isinstance(keyword, list) else keyword)
            self.save_to_mysql(
                host="###t",#host maybe 'localhost'
            ...
            time.sleep(250)  # Add delay between different hashtags
    ...
    def fetch_tweets(self, start_date, end_date):
        ...
        while collected_tweets < self.max_tweets:
            try:
                time.sleep(10)  # delay for loading
...
if __name__ == "__main__":
    keywords = [['trump2025','notrump','potus','uspolitics','vpotus']]  #Add the hashtags we want to search for
